...
df = pd.read_csv("../data/dgs10_clean.csv", index_col="date", parse_dates=True)
...
plt.plot(r_hist.index, traj1, color='blue', label=f'Expected Trajectory from r0={r0_1:.3f}')
plt.plot(r_hist.index[start_idx:], traj2, color='purple', label=f'Expected Trajectory from r0={r0_2:.3f}')
plt.axhline(r_avg, color='grey', linestyle='--', label=f'Historical Average {r_avg:.4f}')
plt.xlabel('Date')
